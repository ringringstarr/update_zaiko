...
if uploaded_file is not None:
    with st.spinner('在庫データを比較・更新しています...'):
        try:
            ...
            TARGET_ROOMS = [
                '330 露付', '330　露付',  # 半角・全角スペースの両方に対応
                '11 半露', '12 半露', '13 半露', '01 半露', 
                '02 半露', '03 半露', 
                '223源泉内付'           # こちらもエクセルの表記に合わせて変更
            ]
...
with st.expander("使用方法", expanded=False):
    ...
    **新滝の温泉付き客室の在庫の増減表を自動で作成するツールです。**
    ...
    3. その後、再度また `シェアフォルダ` にファイルを戻し、上書きしてください。
    ...
    * **在庫の増減判定ルール**
      ...
      * キャンセルが出た場合はセルに「**キャンセル**」と入力されます。
      ※キャンセル箇所は手動でランク数字を入力してください。
    * **エラーチェック**
      手入力による誤入力が発生した場合、「**要確認**」のアラートがでます。
      ※手動で修正してください。
    """)
